...
try {
    ...
    $response = Invoke-RestMethod -Uri "http://localhost:3000/tenant?tenantId=test-tenant" -Method Put -Body $tenantBody -ContentType "application/json"
    Write-Host "Tenant PUT Response: $($response | ConvertTo-Json -Depth 10)"

    # Test Tenant GET
    $response = Invoke-RestMethod -Uri "http://localhost:3000/tenant?tenantId=test-tenant" -Method Get
    ...
    $response = Invoke-RestMethod -Uri "http://localhost:3000/tenant/data?tenantId=test-tenant&year=2023" -Method Put -Body $dataBody -ContentType "application/json"
    Write-Host "Tenant Data PUT Response: $($response | ConvertTo-Json -Depth 10)"

    # Test Tenant Data GET
    $response = Invoke-RestMethod -Uri "http://localhost:3000/tenant/data?tenantId=test-tenant&year=2023" -Method Get
