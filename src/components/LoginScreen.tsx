...
export const LoginScreen = ({ onLoginSuccess, onSpreadsheetIdSubmit, initialToken }: LoginScreenProps) => {
    ...
    // 1. Initial Check: Do we have an active session or encrypted key?
    useEffect(() => {
        ...
        if (activeSessionKey && storedSheetId) {
            // Auto-login with active session
            setSheetId(storedSheetId);
            try {
                const key = JSON.parse(activeSessionKey);
                GoogleSheetsService.loginWithServiceAccount(key);
                onSpreadsheetIdSubmit(storedSheetId);
                onLoginSuccess('service-account');
            } catch (e) {
                // If invalid, fall back to PIN or OAuth
        ...
        } else if (storedKey && storedSheetId) {
            setEncryptedKey(storedKey);
            setSheetId(storedSheetId);
            setAuthStep('pin_entry');
        } else {
    ...
    const createNewSheet = async () => {
        ...
        try {
            ...
            // Check config, but don't block. Go to PIN setup.
            const config = await GoogleSheetsService.checkAppConfig(res.spreadsheetId);
            if (config.status === 'ready' && config.key) {
                setServiceAccountKey(config.key);
            }
            // Proceed to PIN setup even if config is missing (user can paste it while on PIN screen)
            setAuthStep('pin_setup');
    ...
    const handlePinSet = async (pin: string) => {
        setAuthStep('loading');
        try {
            let keyToUse = serviceAccountKey;

            // If we don't have the key yet (e.g. from create flow), try to fetch it again
            if (!keyToUse) {
                const config = await GoogleSheetsService.checkAppConfig(sheetId);
                if (config.status === 'ready' && config.key) {
                    keyToUse = config.key;
                    setServiceAccountKey(config.key);
                } else {
                    throw new Error('Please paste the Service Account JSON in the AppConfig sheet first.');
                }
            }

            const jwe = await GoogleSheetsService.encryptKey(keyToUse, pin);
            localStorage.setItem('service_account_key', jwe);
            localStorage.setItem('spreadsheet_id', sheetId);

            // Store active session
            localStorage.setItem('active_session_key', JSON.stringify(keyToUse));

            // Login immediately
            await GoogleSheetsService.loginWithServiceAccount(keyToUse);
    ...
    const handlePinUnlock = async (pin: string) => {
        ...
        try {
            ...
            // Store active session
            localStorage.setItem('active_session_key', JSON.stringify(key));
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4 transition-colors">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-md w-full p-8 text-center transition-colors">
                ...
                <button
                    onClick={() => login()}
                    className="w-full bg-white text-slate-600 border border-slate-200 dark:border-slate-700 dark:text-slate-200 dark:bg-slate-800 py-3 rounded-xl font-medium hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors flex items-center justify-center gap-3"
                >
